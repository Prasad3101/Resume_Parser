...
def sendmail(res):
    ...
    if match > 50:
        ...
        (pd.DataFrame.from_dict(data=res["data"], orient='index').to_csv(resume_path+'Accepted_profile.csv', 'a', header=False))

        # with open(resume_path+'Accepted_profile.csv', 'a') as csv_file:  
        #     writer = csv.writer(csv_file)
        #     for key, value in res["data"].items():
        #         writer.writerow([key, value])

        # f = open(resume_path+"Accepted_profile.csv",'a')
        # w = csv.DictWriter(f,res["data"].keys())
        # w.writerow(res["data"])
        # f.close()
